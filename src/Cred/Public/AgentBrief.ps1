...
function Get-CredAgentBrief {
    ...
    [void]$sb.AppendLine('```')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('**To verify a value you already have a candidate for, with zero characters of the real one crossing in either direction:**')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('```')
    [void]$sb.AppendLine("echo `"`$candidate`" | cred get $($ctx.Name)/<key> --check")
    [void]$sb.AppendLine('```')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('Prints `match` or `no match` and exits 0/1. The candidate must come via stdin, never as an argument — an argument lands in shell history.')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('**To sanity-check a value without holding it** — catch an empty paste or an obviously wrong one:')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('```')
    [void]$sb.AppendLine("cred get $($ctx.Name)/<key> --stat      # length and character classes, no characters")
    [void]$sb.AppendLine("cred get $($ctx.Name)/<key> --reveal partial   # a few trailing characters and the length")
    [void]$sb.AppendLine('```')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('**Only when a value must actually be read** (and then treat the output as poison — do not echo it back). `--reveal full` is an explicit name for this; the bare command does the same thing:')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('```')
    [void]$sb.AppendLine("cred get $($ctx.Name)/<key>")
    [void]$sb.AppendLine("cred get $($ctx.Name)/<key> --reveal full")
    [void]$sb.AppendLine('```')
    [void]$sb.AppendLine()
    [void]$sb.AppendLine('A `file`-kind credential (a cert, a key, a keytab) has no meaningful masked or partial form, so `--check`, `--stat` and `--reveal partial` all refuse it — use `--reveal full` or `--out <path>` instead.')
    [void]$sb.AppendLine()
